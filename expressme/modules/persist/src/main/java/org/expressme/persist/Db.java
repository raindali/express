...
import org.expressme.persist.mapper.LongRowMapper;
...
public class Db {
    ...
    final static RowMapper<Long> longRowMapper = new LongRowMapper();

    final static RowMapper<Integer> intRowMapper = new IntegerRowMapper();
    ...
    EntityOperation<?> getEntityOperation(Class<?> entityClass) {
    	...
    	if (actualEntityClass==null)
    		throw new DataAccessException("Unknown entity: " + entityClass.getName());
    ...
    public int updateProperties(Object entity, String... properties) {
        if (properties.length == 0)
            throw new DataAccessException("Update properties required.");
        EntityOperation<?> op = getEntityOperation(entity.getClass());
    ...
    public long selectForLong(String sql, Object... args) {
        ...
        if (list.isEmpty())
            throw new DataAccessException("empty results.");
        if (list.size() > 1)
            throw new DataAccessException("non-unique results.");
        return list.get(0);
    }
