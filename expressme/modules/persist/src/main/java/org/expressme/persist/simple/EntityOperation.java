...
import org.expressme.persist.simple.annotations.Column;
import org.expressme.persist.simple.annotations.Id;
import org.expressme.persist.simple.annotations.Table;
import org.expressme.persist.simple.annotations.Transient;
...
class EntityOperation<T> {
	...
	public EntityOperation(Class<T> clazz) {
		this.clazz = clazz;
		this.entityClass = Utils.getEntityClass(clazz);
			...
			public T mapRow(ResultSet rs, String[] names, int[] types) throws SQLException {
				try {
					...
					for (int i = 1; i <= columns; i++) {
						...
						if (value != null) {
							PropertyMapping pm = mappings.get(Utils.delUnderscores(meta.getColumnName(i)));
							if (pm != null) {
								pm.set(t, value);
							}
						}
	...
	String findIdProperty(Map<String, Method> getters) {
		...
		for (String property : getters.keySet()) {
			...
			if (id != null) {
				if (idProperty != null)
					throw new DataAccessException("Duplicate @Id detected.");
				idProperty = property;
			}
		}
		if (idProperty == null)
			throw new DataAccessException("Missing @Id.");
		return idProperty;
	}
	...
	Map<String, PropertyMapping> getPropertyMappings(Map<String, Method> getters, Map<String, Method> setters) {
		...
		for (String property : getters.keySet()) {
			Method getter = getters.get(property);
			if (Utils.getAnnotation(getter, Transient.class) != null)
				continue;
			Method setter = setters.get(property);
			if (setter == null)
				throw new DataAccessException("Missing setter while getter " + getter.getName() + " found.");
	...
	SQLOperation updateProperties(Object entity, String... properties) throws Exception {
		...
		for (String property : properties) {
			PropertyMapping pm = mappings.get(property);
			if (!pm.updatable)
				throw new DataAccessException("Could not update property " + property + " because its updatable=false.");
			sb.append(pm.columnName).append("=?,");
		}
		// delete last ',':
		sb.deleteCharAt(sb.length() - 1);
...
class PropertyMapping {
	...
	public PropertyMapping(Method getter, Method setter) {
		...
		Column column = Utils.getAnnotation(getter, Column.class);
		this.insertable = column == null ? true : column.insertable();
		this.updatable = column == null ? true : column.updatable();
		this.columnName = column == null ? Utils.addUnderscores(Utils.getGetterName(getter)) : (""
				.equals(column.name()) ? Utils.addUnderscores(Utils.getGetterName(getter)) : column.name());
		this.id = Utils.getAnnotation(getter, Id.class) != null;
	}
