...
class Dispatcher {
	...
	void addActions(Object bean) {
		...
		Mapping mappingClazz = clazz.getAnnotation(Mapping.class);
		String urlClazz = mappingClazz == null ? "" : mappingClazz.value();
		Method[] ms = clazz.getMethods();
		for (Method m : ms) {
			if (isActionMethod(m)) {
				Mapping mapping = m.getAnnotation(Mapping.class);
				String url = urlClazz + mapping.value();
	...
	boolean isActionMethod(Method m) {
		...
		//			if (!(converterFactory.canConvert(argType) || argType.isAnnotationPresent(ModelAttribute.class))) {
	...
	void handleExecution(Execution execution, HttpServletRequest request, HttpServletResponse response)
	...
	void handleException(HttpServletRequest request, HttpServletResponse response, Exception ex)
			throws ServletException, IOException {
		try {
